...
namespace Auction_Bussines.Concrete
{
    public class BidService : IBidService
    {
        ...
        public async Task<ApiResponse> CreateBid(CreateBidDTO model)
        {
            ...
            response.isSucces = false;
            response.ErrorMessages.Add("Oooops ! something went wrong");
            return response;
        }
            ...
            {
                var result= await context.Bids.Where(x=>x.BidId == bidId).FirstOrDefaultAsync();
                if (result == null)
                {
                    response.isSucces = false;
        ...
        public async Task<ApiResponse> UpdateBid(int bidId, UpdateBidDTO model)
        {
            ...
            {
                response.isSucces = false;
                response.ErrorMessages.Add("You are not allowed to enter a lower bid amount than your previous bid. Your previous bid amount is: " + result.BidAmount);
                return response;
            }
            response.isSucces = false;
